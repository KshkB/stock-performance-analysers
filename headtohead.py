...
def priceDateVerifySub(stockPrice_data, date):
    ...
    performance[stock_A] = 0
    performance[stock_B] = 0

    for i, d in enumerate(dates):
        if i < len(dates)-1:
            start_i = d
            end_i = dates[i+1]
            returns_A = totalReturns(stock_A, start_i, end_i)
            returns_B = totalReturns(stock_B, start_i, end_i)

            if returns_A[0] > returns_B[0]:
